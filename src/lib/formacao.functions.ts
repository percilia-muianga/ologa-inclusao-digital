...
export const submeterQuiz = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: perfil } = await supabase
      ...
      .maybeSingle();
    if (perfil?.papel !== "formando") {
      return { pontuacao, total, modo_pre_visualizacao: true, _debug: { modulo_id: modulo.id, perguntas_len: lista.length, err: perguntasErr ? JSON.stringify(perguntasErr) : null } };
    }
