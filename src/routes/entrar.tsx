...
function EntrarPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const { data: perfil } = await supabase
      ...
      .eq("id", data.user.id)
      .maybeSingle();
    if (perfil?.papel === "admin_ologa") navigate({ to: "/ologa" });
    else {
      await supabase.auth.signOut();
      setErro("Esta conta não tem acesso à área interna.");
      setLoading(false);
    }
  }
  ...
  return (
    <>
      ...
      <main id="conteudo" className="mx-auto max-w-md px-4 py-16 sm:px-6">
        ...
        <p className="mt-2 text-base text-foreground">Área interna da Ologa.</p>
