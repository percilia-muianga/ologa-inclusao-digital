...
export const LICOES_M3: Record<string, ConteudoLicao> = {
  m3l1: {
    objectivos: [
      ...
      "Aplicar o princípio do menor privilégio escolhendo função e âmbito adequados para cinco pessoas ou sistemas de um serviço público.",
      "Elaborar uma matriz de permissões completa que inclua contas humanas, identidades de aplicação, separação de tarefas, autenticação de dois passos e prazos de revisão e revogação.",
    ],
    explicacao: [
      ...
      "A autenticação de dois passos, também chamada autenticação multifactor, deve ser exigida para todas as contas com permissões administrativas, e é boa prática exigi-la para todas as contas humanas. O segundo factor mais acessível costuma ser uma aplicação geradora de códigos no telemóvel; onde a rede móvel é fraca, as aplicações que geram códigos sem ligação à internet e os códigos de recuperação impressos e guardados em cofre são alternativas a considerar. É preciso ter em conta a acessibilidade: pessoas com baixa visão ou com dificuldade motora podem precisar de mais tempo para introduzir códigos, de códigos com contraste adequado ou de um método alternativo previamente combinado. Um segundo factor que exclui parte dos trabalhadores não é segurança, é barreira.",
    ...
    sintese: [
      ...
      "Dois passos de autenticação para todas as contas administrativas, com alternativa acessível combinada.",
    ...
    guiao: {
      ...
      conducao: [
        ...
        "Exposição: autenticação e autorização; identidade, função e âmbito; hierarquia do âmbito; menor privilégio; separação de tarefas; dois passos com alternativa acessível; contas humanas e identidades de aplicação; revisão e revogação.",
      ...
      errosComuns: [
        ...
        "Confundir autenticar com autorizar: achar que exigir dois passos resolve o excesso de permissões.",
