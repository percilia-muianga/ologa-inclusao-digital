...
const UA: Referencia = {
  ...
  consultadoEm: CONSULTA,
  resumo:
    "A União Africana disponibiliza uma Estratégia Continental para a Inteligência Artificial. Uma estratégia é um documento de orientação política: define prioridades comuns e propõe caminhos. Não é lei, mas também não se deve dizer que uma estratégia nunca obriga a nada: o efeito depende do acto que a aprova e de quem a aprova — uma estratégia adoptada por um governo ou por uma direcção pode vincular administrativamente os serviços que dela dependem, através de instruções, planos ou despachos. No caso desta estratégia continental, ela orienta os Estados e não cria, por si, obrigações directas para um serviço distrital moçambicano. O documento aborda o aproveitamento da inteligência artificial para o desenvolvimento do continente, a criação de capacidades — pessoas formadas, dados, infra-estrutura e capacidade de cálculo —, a atenção aos riscos e a governação, e a cooperação entre Estados africanos, incluindo a preocupação de que África participe na definição das regras internacionais e não apenas as receba. Para uma direcção distrital ou provincial, o valor prático está em situar decisões locais num quadro continental: quando se escolhe uma ferramenta, faz sentido perguntar onde ficam os dados, que dependência se cria face a um fornecedor estrangeiro, e se existe alternativa que desenvolva competências internas. Não se deve citar esta estratégia como se fosse legislação nem como aprovação de qualquer solução concreta.",
};
