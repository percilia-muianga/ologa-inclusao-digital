...
const M2L1: ConteudoLicao = {
  ...
  explicacao: [
    "A maior parte das falhas graves em aplicações web não é exótica: é controlo de acesso mal feito. O padrão repete-se — a aplicação esconde o botão na interface, mas o pedido continua a funcionar se for feito directamente; ou usa o identificador que vem do cliente para decidir que registo mostrar, sem verificar se aquele utilizador tem direito àquele registo. A regra é simples de enunciar: a autorização verifica-se no servidor, a cada pedido, com base na identidade da sessão, e nunca em algo que o cliente possa alterar.",
  ...
  laboratorio: {
    titulo: "Dois testes autorizados de controlo de acesso em APP-LAB",
    objectivo:
      "Executar, na aplicação vulnerável de laboratório, os casos de teste de O-01 e O-03 escritos na actividade, recolher evidência e registá-la no formato do relatório.",
    recursos: [
      "Máquina virtual «APP-LAB» com uma aplicação web deliberadamente vulnerável para formação, instalada localmente a partir de imagem preparada pelo formador, sem qualquer ligação à internet.",
      "Máquina virtual «EST-ADMIN» com navegador e ferramenta de inspecção de pedidos (as ferramentas de programador do próprio navegador são suficientes).",
      "Duas contas de demonstração criadas pelo formador em APP-LAB, «utilizador1» e «utilizador2», com um pedido cada, com dados fictícios.",
    ...
    preparacao: [
      "Instalar e arrancar APP-LAB no dia anterior, confirmar que a aplicação abre e criar os pedidos de demonstração das duas contas.",
      "Tirar instantâneo «inicial» de APP-LAB com a aplicação já semeada com os dados fictícios.",
      "Confirmar que a rede virtual está isolada e que APP-LAB não alcança a internet nem a rede da sala.",
      "Ler em voz alta a autorização do anexo B antes de qualquer teste e confirmar que todos entendem os alvos excluídos.",
    ],
    passos: [
      "Entrar em APP-LAB como «utilizador1» e anotar o número do próprio pedido.",
      "Executar o caso de teste de O-01: alterar o número na barra de endereço para o pedido de «utilizador2» e registar o que a aplicação devolve, com captura de ecrã e hora.",
      "Repetir a mesma alteração depois de terminar a sessão, para distinguir falha de autorização de falha de autenticação, e registar o resultado.",
      "Executar o caso de teste de O-03: aceder a /admin sem sessão iniciada e registar se a página abre, que funcionalidades mostra e que evidência foi recolhida.",
    ...
    reversao: [
      "Restaurar o instantâneo «inicial» de APP-LAB no fim da sessão.",
      "Confirmar que os pedidos de demonstração voltaram ao estado original.",
      "Apagar as capturas de ecrã das máquinas partilhadas depois de anexadas à folha do par.",
    ],
    alternativaOffline: [
      "Trabalhar sobre o conjunto impresso de pedido e resposta fornecido pelo formador para O-01, identificando a linha do pedido que decide o registo devolvido.",
